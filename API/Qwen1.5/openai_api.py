...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    # 将 top_p 固定为 0.9，忽略请求中的 top_p 设置
    gen_kwargs = {'top_p': 0.9}

    # 继续处理 temperature 参数
    if request.temperature is not None:
        if request.temperature < 0.01:
            gen_kwargs['top_k'] = 1  # 贪婪解码
        else:
            gen_kwargs['temperature'] = request.temperature

    # 使用 gen_kwargs 进行模型推理
...
def _get_args():
    ...
    parser.add_argument(
        ...
        "--checkpoint-path",
        type=str,
        default="/usr/local/TFBOYS/gc/model/qwen/Qwen1.5-7B-Chat",
    ...
    parser.add_argument(
        "--server-port", type=int, default=9015, help="Demo server port."
    )
    parser.add_argument(
        "--server-name",
        type=str,
